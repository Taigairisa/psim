...
def launch_viewer(model):
    ...
    if not HAVE_PYSIDE6:
        print("GUI cannot be launched because PySide6 is not installed.")
    ...
    class ViewerWindow(QMainWindow):
        ...
        def draw_model(self):
            ...
            for node in self.model.nodes:
                if hasattr(node, 'pos') and node.pos:
                    ...
                    self.node_items[node.name] = rect_item

            # Second, draw all connections
            for node in self.model.nodes:
                if hasattr(node, 'out') and node.out and node.name in self.node_items and node.out.name in self.node_items:
                    from_item = self.node_items[node.name]
                    to_item = self.node_items[node.out.name]
        ...
        def on_entity_created(self, timestamp, entity_id, entity_name, node_name):
            ...
            if node_name in self.node_items:
                ...
                entity_item = QGraphicsRectItem(0, 0, 10, 10) # Using rect for now
...
if __name__ == "__main__":
    if HAVE_PYSIDE6:
        ...
        print("Cannot run viewer standalone without PySide6.")
        print("Please install it with: pip install pyside6")
